<h2 class="alert alert-success">Xuelin's tools</h2>

#### Todo
* initial route changes
    * Rename orignal to /builtin
    * Create new /cmd
    * Default / can switch to one of those easily
...
The `home-routes` handler in the `edw.routes.home` namespace
...
The `home-page` function will in turn call the `edw.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/edw/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `edw.handler` namespace:
...
Request middleware functions are located under the `edw.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `edw.dev-middleware` namespace found in
